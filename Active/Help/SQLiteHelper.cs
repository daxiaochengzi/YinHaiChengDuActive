...
/// </summary>
    public  class SqLiteHelper
    {
        ...
        {
            string path = @"C:\Program Files (x86)\Microsoft\本鼎医保插件\logData.db";
            //SQLiteConnection.CreateFile(path);
            SQLiteConnection cn = new SQLiteConnection("data source=" + path);
            cn.Open();
            cn.Close();
        }
        public static  void NewTableData()
        {
            string dbPath = @"C:\Program Files (x86)\Microsoft\本鼎医保插件\logData.db";
            ...
            if (sqliteConn.State !=ConnectionState.Open)
            {
                ...
                cmd.CommandText = "CREATE TABLE Data (OperatorId nvarchar(100),JoinJson Text, ReturnJson Text,CreateTime Text,TransactionCode nvarchar(100))";
                cmd.ExecuteNonQuery();
            }
            sqliteConn.Close();
        }

        public static void NewTableDataError()
        {
            string dbPath = @"C:\Program Files (x86)\Microsoft\本鼎医保插件\logData.db";
            SQLiteConnection sqliteConn = new SQLiteConnection("data source=" + dbPath);
            if (sqliteConn.State != ConnectionState.Open)
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand();
                cmd.Connection = sqliteConn;
                cmd.CommandText = "CREATE TABLE DataError (OperatorId nvarchar(100),JoinJson Text, ReturnJson Text,CreateTime Text,TransactionCode nvarchar(100))";
                cmd.ExecuteNonQuery();
            }
            sqliteConn.Close();
        }
